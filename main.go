...
// distance возвращает дистанцию (км), которую преодолел пользователь.
// Формула расчета:
...
func (t Training) TrainingInfo() InfoMessage {
	...
	iMsg.TrainingType = t.TrainingType
	iMsg.Duration = t.Duration
	iMsg.Distance = t.distance()
	iMsg.Speed = t.meanSpeed()
	iMsg.Calories = t.Calories()
	return iMsg
}
...
type Swimming struct {
	// добавьте необходимые поля в структуру
	Training
	LengthPool int // длина бассейна (м)
	CountPool  int // количество пересечений бассейна (разы)
}
